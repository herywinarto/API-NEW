...
def YoutubeSearch():
    ...
    main = json.loads(requests.get("https://api.be-team.me/ytdl2?search="+search,headers=headers).text)
...
def YoutubeURL():
    ...
    headers = {"apiKey": apiKey} ## APIKEY, YOU CAN BUY FROM ME IN WHATSAPP: +6289625658302
    main = json.loads(requests.get("https://api.be-team.me/ytdl2?url="+url,headers=headers).text)
    print(json.dumps(main, indent=4, sort_keys=True))
